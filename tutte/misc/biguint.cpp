...
uint32_t biguint::c_uint32() const
{
  if (ptr & BUI_LEFTMOST_BIT)
  {
    ...
    if (p[0] > 1U)
    {
      throw runtime_error("biguint too large for uint32_t");
    }
    return p[2];
  }
  else
...
uint64_t biguint::c_uint64() const
{
  if (ptr & BUI_LEFTMOST_BIT)
  {
    ...
    if (depth > 2U)
    {
      throw runtime_error("biguint too large for uint64_t");
    }
    uint64_t r = 0;
    for (uint32_t i = depth + 1; i > 1; --i)
    {
...
uint32_t *biguint::aligned_alloc(uint32_t c)
{
  ...
  if (((uint32_ptr_t)p) & 1U)
  {
    throw std::runtime_error("Allocated memory not aligned!");
  }
  return p;
}
